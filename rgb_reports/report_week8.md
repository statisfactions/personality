...
This connects to the broader knowledge-action gap literature on LLM representations (Basu et al. 2026 arXiv:2603.18353, the canonical "knowledge-action gap" paper; Wu et al. 2026 "Knowing without Acting" arXiv:2603.05773 demonstrating causal double-dissociation between recognition and execution subspaces; CARE 2024; see `rgb_reports/lit_review_steering_vectors.md` §4 for the full landscape). Where they find recognition and execution use different subspaces, we find a finer-grained version on the *reading* side specifically: even within "recognition" (rep readout), the geometry is contingent on the extraction vocabulary. The model can SYMBOLICALLY recognize across vocabularies; the LINEAR PROJECTION can't. This may extend the knowledge-action taxonomy: there's reading-vs-reasoning as a related dissociation that lives upstream of the probing-vs-steering split.
