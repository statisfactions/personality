...
Heatmaps: `results/persona/persona_repr_heatmap_ipip_raw_*.html` (cohort rep, 7 models), `results/persona/persona_likert_heatmap_ipip_raw_*.html` (Qwen7 only).
...
Heatmaps: `results/persona/persona_likert_heatmap_ipip_raw_target_ipip_*.html` (Qwen7).
...
Heatmaps: `results/persona/persona_repr_heatmap_ipip_full_*.html`, `results/persona/persona_likert_heatmap_ipip_full_*.html` (cohort, post-§6).
...
This connects to the broader probing-vs-execution literature on LLM representations (Wu et al. 2026 "Knowing without Acting" arXiv:2603.05773, demonstrating causal double-dissociation between recognition and execution subspaces; CARE 2024; "knowledge-action gap" framing per Basu et al. 2026 arXiv:2603.18353, though the methodology is more suggestive than dispositive; see `rgb_reports/lit_review_steering_vectors.md` §4 for the full landscape). Where these papers find recognition and execution use different subspaces, we find a finer-grained version on the *reading* side specifically: even within "recognition" (rep readout), the geometry is contingent on the extraction vocabulary. The model can SYMBOLICALLY recognize across vocabularies; the LINEAR PROJECTION can't. This may extend the dissociation taxonomy: there's reading-vs-reasoning as a related split that lives upstream of the probing-vs-steering one.
...
Cohort reflow heatmaps available at `results/persona/persona_repr_heatmap_ipip_reflowed_full_*.html` and `persona_likert_heatmap_ipip_reflowed_full_*.html` (matched-condition: reflowed persona × IPIP dir × IPIP target).

### 8.8. Headline figures

`scripts/persona_w8_summary_plot.py` generates two summary HTML figures:

- `results/persona/persona_w8_trajectory.html` — cohort mean rep + Likert across the 5 conditions (W7 → §3 raw → §4 raw → §5 raw → §5 reflow), with the matched gap shown as bars below. The rep recovers slowly across vocabulary-coupling strip-down; Likert drops; gap shrinks then partially recovers under reflow.
- `results/persona/persona_w8_per_model_gap.html` — per-model matched gap raw → reflow slope plot. Shows the heterogeneity directly: most models grow under reflow, Gemma 4B and Llama 3B shrink slightly, all converge into the +0.01 to +0.12 band. Cohort-mean diamond marker shows aggregate trajectory.
...
- `results/facets/ipip_facet_dashboard.html` — single-page reading-group dashboard. 3 rows: per-model within-vs-across cosine bars (IPIP and HEXACO side-by-side); per-model NN-within-% + cluster purity bars (5-cluster IPIP vs 6-cluster HEXACO); cross-model 7×7 cosine-matrix correlation heatmaps with mean off-diag r in subplot titles.
- `results/facets/ipip_facet_cross_model_heatmap.html` — cross-model 7×7 heatmaps only, larger.
- `results/facets/ipip_facet_cluster.json` and `results/facets/facet_cluster.json` — per-model summaries with full cosine matrices, used by both the dashboard and the cross-model script.
