...
- **(a) Per-model robustness of W14 §4 — highest priority.** The −0.83 (human PC1 ≈ model PC1), the two-pole split, "shared PC1 carries ~70% of the match," and the PC↔Big-Five correlations are all cohort-mean. Averaging 12 heterogeneous models (4B–35B + Mamba + multilingual SFT) can manufacture *or* wash out structure; per-model relational r already ranges 0.40 (Phi4) → 0.64 (Qwen7). Want a bootstrap-over-models CI on the −0.83 and the pole congruence, and confirm the two-pole structure is present *per model*, not an averaging artifact. This is the "are we fooling ourselves" pass we ran for the 6th factor (W14 §1) and skipped for §4.
