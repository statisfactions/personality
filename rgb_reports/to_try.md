...
**Status.** Not started. Cheap — their model + our facet map + our existing human matrix; no inference runs. Highest-value because it retroactively calibrates the entire §7/§8 facet-recovery claim, and bears directly on the superposition-vs-word-embedding-geometry question (`memory/user_superposition_vs_embedding.md`). Natural companion to #18 (chunking) and #5 (SAE) — all three ask "what is the facet structure really a fact about."
