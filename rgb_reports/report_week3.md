...
Ran the full validation suite across all 4 models. The central finding: **representation, binary-choice, and free-text measure three distinct constructs that don't agree.** This has implications for what "model personality" means and how it should be measured.
...
| Model | BC ↔ Free-text agree | RepE ↔ BC r | Interpretation |
|---|---|---|---|
...
Phi4-mini — the model with the cleanest RepE separation (100% LDA, PC1 = personality) and highest Likert reliability (ICC 0.77) — shows only 40% agreement between binary-choice and free-text. The model that "knows" honesty best is the least consistent in how it expresses it.

Qwen's negative RepE↔BC correlation (r=-0.61) means items represented as "more honest" internally are less likely to get honest binary-choice picks. This is a genuine read/write dissociation.

## 2. Three Constructs, Not One

The validation reveals that "model personality" is not one thing:

| Property | Representation (RepE) | Binary-Choice | Free-Text |
|---|---|---|---|
...
| **Predicts other formats?** | No (r≈0 with Likert, ±0.3 with BC) | 40-80% agree with free-text | — |
...
**For safety/alignment:** Output behavior (BC or free-text) is format-dependent. A model that picks the "honest" option in binary-choice may not generate honest free-text, and vice versa. Testing alignment in one format doesn't guarantee it holds in another.
...
**For the trait-conflict instrument:** The binary-choice ceiling effects (H/C/O all near 100%) confirm that single-trait scenarios are too easy. Trait-conflict dilemmas (H vs A, C vs O) are necessary to generate between-model variation where RLHF doesn't prescribe an answer.
...
These are complementary, not competing. The internal representation is stable, but the mapping from representation to behavior is format-dependent. The gap between them IS the personality measurement problem — we can measure what the model "knows" (RepE) or what it "does" (BC/free-text), but these aren't the same thing.
