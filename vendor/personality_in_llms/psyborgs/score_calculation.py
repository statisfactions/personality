...
def normalize_response_logprobs(df):
    ...
    raw_response_scores_normalized_df = pd.DataFrame({
        ...
        'response_value': logprobs_sum.index.get_level_values('model_output').astype(int),
...
def reshape_response_choice_probability_scores(
    raw_response_scores_df: pd.DataFrame) -> pd.DataFrame:
  ...
  Args:
    raw_response_scores_df: A DataFrame containing raw response scores in long
      format, with one unique prompt-continuation (containing a single
      response choice) and an LLM score for this unique combination for each
      row. Columns should include `item_preamble_id`, `item_id`,
      `item_postamble_id`, `response_scale_id`,
      `response_choice_postamble_id`, `model_id`, and `response_value`.
  ...
  df_raw_wide = raw_response_scores_df.pivot_table(
      index=SPID, columns=['item_id', 'response_value'], values=['score'])
...
def calculate_human_simulated_responses(
  ...
  """Selects the most likely response choices to simulate human responses.
    ...
    Args:
      raw_response_scores_df: A DataFrame containing raw response scores in
        long format, with one unique prompt-continuation (containing a single
        response choice) and an LLM score for this unique combination for each
        row. Columns should include `item_preamble_id`, `item_id`,
        `item_postamble_id`, `response_scale_id`,
        `response_choice_postamble_id`, `model_id`, and `response_value`.
  ...
  print(f'Determining the most likely response choice per item. ')
  ...
  df_item_responses = raw_response_scores_df.loc[
    raw_response_scores_df.groupby(
      SPID + ['item_id'])['score'].idxmax()].reset_index(drop=True)

  # reshape to wide
  df_simulated_item_responses_wide = df_item_responses.pivot(
      index=SPID, columns=['item_id'],
      values='response_value').reset_index().rename_axis(
...
def get_raw_and_simulated_responses(
  raw_response_scores_df: pd.DataFrame,
  generative_mode: bool = False) -> pd.DataFrame:
  ...
  if generative_mode:
    df_raw_response_scores_df = normalize_response_logprobs(
      df_raw_response_scores_df)
  
  # reshape raw LLM response choice scores
  print(f'Reshaping raw LLM response choice scores... ')
  ...
  print(f'Calculating and reshaping human-simulated item responses... ')
  df_simulated_item_responses = calculate_human_simulated_responses(
      raw_response_scores_df)

  # combine the above into one DataFrame
  print(
    'Combining LLM scores and human-simulated responses into one DataFrame... '
  )

  return df_simulated_item_responses.merge(
      df_raw_reshaped, how='inner')


# calculate session scale scores
def score_session(admin_session: survey_bench_lib.AdministrationSession,
                  raw_response_scores_df: pd.DataFrame,
  ...
  This function treats each unique prompt-continuation specification as a
    simulated participant, indexed by a simulated participant ID (SPID).
    Iterating through each multi-item scale in an AdministrationSession, it
    calculates a summary score for each SPID by taking the average of all
  ...
  Args:
    ...
    raw_response_scores_df: A DataFrame containing raw response scores in long
      ...
      choice) and an LLM score for this unique combination for each row.
      Columns should include `item_preamble_id`, `item_id`,
      `item_postamble_id`, `response_scale_id`, `response_choice_postamble_id`,
      `model_id`, and `response_value`.
  ...
  # for each scale, score simulated participants (SPIDs)
    ...
    for scale_id, scale in measure.scales.items():
      ...
      for response_scale_id in response_scale_ids:
        ...
        for item_id in item_ids:
          original_values = scored_session_df[df_response_scale_id_col ==
                                              response_scale_id][item_id]
        ...
        if verbose:
          print('Simulated \"' + response_scale_id + '\" scale scores for ' +
                scale_id + ': \n' + str(simulated_scale_scores) + '\n')

        scored_session_df.loc[df_response_scale_id_col == response_scale_id,
                              scale_id] = simulated_scale_scores
