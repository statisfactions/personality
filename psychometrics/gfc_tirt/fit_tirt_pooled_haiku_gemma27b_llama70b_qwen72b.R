#!/usr/bin/env Rscript
# Pooled Okada-style TIRT fit: Haiku 4.5 + Gemma3-27B + Llama3.3-70B +
# Qwen2.5-72B. Stress-test of the 7-category-pooling assumption: Qwen2.5-72B
# uses an effectively-5-category subset (1/3/4/5/7, with 2 and 6 nearly
# absent), so this pool is documented as a heterogeneity case rather than
# a clean extension of the 3-model pool. Compare per-trait recovery
# against the 3-model pool to see whether shared κ/a_j absorbs the cost.
#
# Usage:
#   Rscript psychometrics/gfc_tirt/fit_tirt_pooled_haiku_gemma27b_llama70b_qwen72b.R \
...
output_path <- if (length(args) >= 1) args[1] else "psychometrics/gfc_tirt/pooled_haiku_gemma27b_llama70b_qwen72b_fit.rds"
