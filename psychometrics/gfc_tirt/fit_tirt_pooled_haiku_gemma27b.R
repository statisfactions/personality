#!/usr/bin/env Rscript
# Pooled Okada-style TIRT fit limited to Haiku 4.5 + Gemma3-27B.
#
# Rationale: the 5-model pooled fit suffered from response-style heterogeneity
# (Phi4 skips cat 5, Gemma3-4B skips cat 5, Haiku uses all 7 smoothly).
# Restricting to the two strongest models — both of which use the full
# 7-category range — should let shared κ thresholds and a_j discriminations
# fit both without compromise.
#
# Usage:
#   Rscript psychometrics/gfc_tirt/fit_tirt_pooled_haiku_gemma27b.R \
...
output_path <- if (length(args) >= 1) args[1] else "psychometrics/gfc_tirt/pooled_haiku_gemma27b_fit.rds"
