...
# Load response data (wide format) — Phi4-mini synthetic personas
wide <- read_csv("results/phi4-mini_gfc30_synthetic_wide.csv", show_col_types = FALSE)
...
cat("Fitting TIRT Stan model (cumulative family, Phi4-mini, 400 respondents)...\n")
cat("Using 4 chains x 1000 iter (500 warmup)\n\n")
...
# Save fit object
saveRDS(fit, "results/phi4_gfc30_synthetic_tirt_fit.rds")
cat("Fit saved to results/phi4_gfc30_synthetic_tirt_fit.rds\n\n")
...
cat("\n=== Simple Scoring (for comparison) ===\n")
raw <- jsonlite::fromJSON("results/phi4-mini_gfc30_synthetic.json", flatten = TRUE)
