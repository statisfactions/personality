#!/usr/bin/env Rscript
# Pooled binary (true forced-choice) TIRT fit on the same 4-model lineup
# as the graded pool: Haiku 4.5 + Gemma3-27B + Llama3.3-70B + Qwen2.5-72B.
# Mirrors fit_tirt_pooled_haiku_gemma27b_llama70b_qwen72b.R but reads the
# *_binary.json inputs and uses tirt_okada_binary.stan.
#
# Usage:
#   Rscript psychometrics/gfc_tirt/fit_tirt_pooled_binary.R \
...
output_path <- if (length(args) >= 1) args[1] else "psychometrics/gfc_tirt/pooled_binary_haiku_gemma27b_llama70b_qwen72b_fit.rds"
...
for (slug in names(MODELS)) {
  for (cond in c("honest", "fakegood", "bare", "respondent")) {
    path <- switch(cond,
      honest     = sprintf("psychometrics/gfc_tirt/%s_gfc30_synthetic_binary.json", slug),
      fakegood   = sprintf("psychometrics/gfc_tirt/%s_gfc30_synthetic-fakegood_binary.json", slug),
      bare       = sprintf("psychometrics/gfc_tirt/%s_gfc30_neutral-bare_binary.json", slug),
      respondent = sprintf("psychometrics/gfc_tirt/%s_gfc30_neutral-respondent_binary.json", slug)
    )
