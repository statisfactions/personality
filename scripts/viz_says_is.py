...
def load():
    ...
    return adjs, says, says_fr, axis


def main():
    adjs, says, says_fr, axis = load()
    ...
    for k, m in enumerate(MODELS):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(size=4, color=BLUE, opacity=0.35,
                        line=dict(width=0)),
            customdata=adjs, hovertemplate="%{customdata}<br>is %{x:.2f} · "
    ...
    fig.update_layout(
        title=dict(text="SAYS vs IS — self-report EV against the model's own "
                        "ablated assistant-axis projection, per adjective "
                        "(labels: biggest over-/under-claims; 'r others' = "
                        "same self-report scored against the other nine "
                        "models' axes)", font=dict(size=13, color=INK)),
