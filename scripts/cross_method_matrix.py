...
MODELS = {
    # All entries HF-based throughout (W7 §17 cleanup, 2026-04-25): chat-template
    # Likert via run_ipip300.py / run_hexaco.py; chat-template BC via score_bc.py;
    # chat-template RepE via repe_legacy_from_cache.py --format chat. The "ollama"
...
def load_bc_scores():
    ...
    Reads `results/binary_choice_6trait.json`, which contains both:
      - small-cohort entries from prior Ollama runs (keyed by Ollama names like
        `H_gemma3:4b`)
      - larger-cohort entries from HF + chat-template (keyed by short names like
        `H_Gemma12`), generated by `scripts/score_bc.py`.
    The lookup key is `<TRAIT>_<MODELS[m]["ollama"]>` for whichever cohort.
    """
    try:
