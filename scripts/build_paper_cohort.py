...
def main():
    ...
    def md_table(rs, caption, label):
        lines = ["| Model | Family | Params (B) | Thinking | Citation |",
                 "|---|---|---|---|---|"]
        for r in rs:
            lines.append(
                f"| {r['name']} | {r['family']} | {r['params_b']} | "
                f"{r['thinking']} | [@{r['cite']}] |")
        lines.append(f"\n: {caption} {{#{label}}}\n")
        return "\n".join(lines)
    ...
    with open(f"{PAPER}/_data/cohort_table_small.md", "w") as fp:
        fp.write(md_table(
            deep_rows,
            "The deep cohort: all five channels (SELF, REPRESENT, JUDGE, "
            "ENACT, and rollout text).", "tbl-cohort-small"))
    lines = ["| Model | Family | B | Gen | Channels | Thinking | Status | Cite |",
             "|---|---|---|---|---|---|---|---|"]
    for r in wide_rows:
        st = r["status"].split(":")[0]
        lines.append(
            f"| {r['name']} | {r['family']} | {r['params_b']} | "
            f"{r['generation']} | {r['channels']} | {r['thinking']} | "
            f"{st} | [@{r['cite']}] |")
    lines.append("\n: The wide cohort. Channels: S = SELF, R = REPRESENT, "
                 "E = default-persona ENACT rollouts, T = thinking-mode SELF "
                 "arm. Status: excluded/flagged rows per the measurement-"
                 "failure taxonomy (see text). {#tbl-cohort-large}\n")
