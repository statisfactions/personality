...
**Author:** ecb · **Date:** 2026-07-09 · **Reports:**
`psychometrics/judge/reports/01–05` (knitted HTML) · **Data:** rgb JUDGE release
(`judge_likert_20260630`, 12 models × 523² conditional-endorsement Likert).
...
leniency (mean 3.6–4.8), differentiation (SD 0.47–1.95) and decisiveness (entropy) each
vary ~4× and are only loosely coupled — three separate scoring nuisances. Two models
(`Llama3.2-3B`, `FalconMamba-7B`) are **range-restricted / near-non-responsive**:
`Llama3.2` effectively uses only categories {4,5}; `FalconMamba` sits near the uniform
entropy ceiling. The sensible-looking part — an inverted-U of per-judgment entropy in
similarity (certain at the poles, graded in the middle) — is present for discriminating
raters and *absent* for these two, which is how we know their flatness is guessing, not
compression. **Scoring implication:** cross-model *magnitude* comparison is unsafe; center
raters (or use consistency-type coefficients) before any pooling.
...
- **D-study:** ~4 raters reach absolute G ≥ 0.8, ~8 reach 0.9; a reactive follow-up
  (Report V) shows a random **~6–7 raters reproduce the full consensus at r > 0.95**. The
  ensemble's value is real but saturates well below 12, and model identity within the
  crowd is largely fungible.
- **Where the unreliability lives** is reassuring: between-rater disagreement is an
  inverted-U in similarity (raters concur on clear synonyms/antonyms, contest the
  middle) and co-locates with within-rater entropy. The uncertainty is **construct-located**
  (theory-laden co-occurrence), not a rater artifact.
...
loo 0.92); the genuine low-agreement outliers are the high-variance **idiosyncratic**
models (`Gemma-4-31B` 0.76, `Aya` 0.83). Rater quality is **multidimensional** —
information, between-rater agreement, and within-rater consistency rank the raters
differently and are ~orthogonal — which is exactly why a single competence weight can't
help. Competence- and confidence-weighting each buy ~0.003 in split-half stability over
the flat mean; trimming either extreme nudges single-rater ICC by *opposite* variance
mechanisms (dropping flat raters raises pair variance 0.91→1.10; dropping idiosyncratic
raters cuts residual 0.43→0.36) while leaving the ensemble untouched. **The flat
unweighted mean of ≥6 discriminating raters is near-optimal.** Simplicity costs nothing.
...
- That structure is **reproducible across raters** (mean Tucker congruence to consensus
  0.85; 0.85 again on the general-factor-removed residual). It is a property of the
...
(PC1/PC2 = 3.75) — a good/competent ↔ bad/incompetent axis pervading every judgment. It
